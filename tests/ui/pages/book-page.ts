...
class BookPage {
  ...
  constructor(page: Page) {
    ...
    this.isbnLabel = page.locator('#ISBN-wrapper #userName-value');
    this.speakingJSBook = page.getByRole('link', { name: 'Speaking JavaScript' });
  ...
  async addToYourCollection(isDupe?: boolean) {
    ...
    await expect(this.addToYourCollectionButton).toBeVisible({ timeout: 15000 });
    await this.addToYourCollectionButton.click();
  }

  async checkSpeakingJSIsbn() {
    await expect(this.page).toHaveURL(/search=9781449365035/, { timeout: 15000 });
    await expect(this.page.locator('#ISBN-wrapper')).toContainText('9781449365035', { timeout: 15000 });
  }
  ...
  async clickAtSpeakingJSBook() {
    await this.speakingJSBook.click({ timeout: 15000 });
  }
